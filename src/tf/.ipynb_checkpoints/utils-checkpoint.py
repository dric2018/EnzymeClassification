...
def seq_to_text_file(save_file_to:str, sequences, max_len=MAX_SEQ_LEN, exist_ok=True):
    if save_file_to.split('/')[-1] in  os.listdir(os.path.dirname(save_file_to)):
        pass
    else:
...
def run_fold(fold_num, model, dataset, save_ckpt_to, epochs, data_dir, log_dir='../../runs/logs', train_bs=1024, val_bs=512):
    ...
    if fold_num != -1 :
        ...
        train = dataset[dataset["FOLD"] !=fold_num]
        validation = dataset[dataset["FOLD"] ==fold_num]
        ...
        split = np.random.rand(len(dataset)) < .8
        train = dataset[split]
        ...
        # create txt files 
        seq_to_text_file(save_file_to=os.path.join(data_dir, f'train_data.txt'), sequences=train.SEQUENCE)
        seq_to_text_file(save_file_to=os.path.join(data_dir, f'validation_data.txt'), sequences=validation.SEQUENCE)
        
        # callbacks 
        model_name = f"enzyme_classifier-full.h5"
        ckpt_cb_1 = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join(save_ckpt_to, model_name),
                                                    save_weights_only=True,
                                                    monitor = 'val_accuracy',
                                                    save_best_only=True,
        ...
        # getting data loaders
        train_data_loader = get_data_loader(file=os.path.join(data_dir, f'train_data.txt'), 
                                            batch_size=train_bs, 
                                            labels=train_labels, 
                                            task='train')

        validation_data_loader = get_data_loader(file=os.path.join(data_dir, f'validation_data.txt'), 
                                                batch_size=val_bs, 
                                                labels=validation_labels, 
                                                task='validation')
